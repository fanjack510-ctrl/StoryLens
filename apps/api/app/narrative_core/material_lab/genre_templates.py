...
# 知识库不是正文索引。一本书无论多长，都只保留能代表该题材创作知识的
# 少量条目；分类内限额负责防止某个高频词淹没目录，全书限额负责防止长篇
# 天然比短篇产生更多“知识”。
BOOK_MATERIAL_LIMIT = 72
CATEGORY_LIMIT_BY_STRATEGY = {
    "hook": 8,
    "object_anomaly": 6,
    "behavior": 6,
    "person_doubt": 6,
...
def _c(key, label, strategy, subs, cues=None, stage="", sort=100) -> dict[str, Any]:
    return {
        ...
        "cues": cues or [],
        "sort": sort,
        "max_items": CATEGORY_LIMIT_BY_STRATEGY.get(strategy, 5),
...
XUANHUAN = [
    ...
    _c("craft_economy", "炼制交易", "state", [
        ("craft", "炼药炼器", ["炼药", "炼丹", "炼器", "锻造", "材料", "成功率"]),
        ("trade", "交易拍卖", ["拍卖", "商会", "交易", "估价", "竞价", "寄售"]),
        ("currency", "货币兑换", ["金币", "晶石", "灵石", "兑换", "价格", "手续费"]),
    ], stage="全书", sort=75),
    _c("faction", "势力", "relation", [
        ("sect", "宗门世家", ["宗门", "世家", "门派", "家族", "长老"]),
        ("hostile", "敌对势力", ["敌对", "仇家", "死敌", "宿敌"]),
    ], stage="全书", sort=80),
    _c("society_rule", "社会制度", "state", [
        ("admission", "选拔准入", ["招生", "考核", "名额", "资格", "入门", "录取"]),
        ("governance", "组织治理", ["议事", "执法", "长老会", "规矩", "管辖", "任命"]),
        ("status", "身份权利", ["身份", "特权", "爵位", "平民", "贵族", "奴隶"]),
    ], stage="全书", sort=85),
    _c("map", "地图秘境", "state", [
        ("secret_realm", "秘境", ["秘境", "遗迹", "洞府", "禁地", "试炼"]),
        ("dungeon", "副本历练", ["历练", "试炼", "大比", "猎场"]),
    ], stage="中段", sort=90),
    _c("cost_rule", "力量代价", "state", [
        ("consumption", "消耗条件", ["消耗", "耗尽", "需要", "代价", "燃烧", "寿命"]),
        ("backlash", "反噬风险", ["反噬", "走火入魔", "经脉", "失控", "虚弱", "副作用"]),
        ("restriction", "使用限制", ["限制", "只能", "不得", "间隔", "条件", "封印"]),
    ], stage="全书", sort=95),
...
XIANXIA = [
    ...
    _c("alchemy_craft", "丹器符阵", "state", [
        ("alchemy", "炼丹", ["炼丹", "丹炉", "火候", "药性", "成丹", "废丹"]),
        ("artifact", "炼器", ["炼器", "祭炼", "器胚", "禁制", "本命法宝", "器灵"]),
        ("talisman_array", "符箓阵法", ["符箓", "画符", "阵法", "阵眼", "布阵", "破阵"]),
    ], stage="全书", sort=45),
    _c("secret_realm", "秘境洞天", "state", [
        ("realm", "秘境", ["秘境", "洞天", "遗府", "禁地"]),
    ], stage="中段", sort=50),
    _c("exchange_economy", "仙门交易", "state", [
        ("merit", "功德贡献", ["贡献", "功德", "门功", "兑换", "任务", "份额"]),
        ("market", "坊市交易", ["坊市", "拍卖", "摊位", "灵石", "价格", "寄卖"]),
    ], stage="全书", sort=55),
    _c("karma", "因果宿命", "structure", [
        ("karma", "因果", ["因果", "宿命", "前世", "轮回", "命数"]),
    ], stage="全书", sort=60),
    _c("spiritual_ecology", "灵脉生态", "state", [
        ("spirit_vein", "灵脉洞府", ["灵脉", "灵气", "洞府", "聚灵", "灵泉", "地脉"]),
        ("spirit_beast", "灵兽灵植", ["灵兽", "妖兽", "灵植", "药园", "成熟", "守护兽"]),
    ], stage="全书", sort=65),
    _c("restriction_cost", "术法代价", "state", [
        ("consumption", "法力神识", ["法力", "灵力", "神识", "消耗", "枯竭", "恢复"]),
        ("backlash", "反噬心魔", ["反噬", "心魔", "走火入魔", "道基", "经脉", "寿元"]),
        ("restriction", "境界限制", ["境界", "只能", "不可", "限制", "封印", "压制"]),
    ], stage="全书", sort=68),
...
DUSHI = [
    ...
    _c("workplace_knowledge", "职场职业", "state", [
        ("role_division", "岗位分工", ["岗位", "部门", "职责", "主管", "助理", "编辑", "经纪人"]),
        ("workflow", "工作流程", ["流程", "交接", "审批", "汇报", "排期", "验收", "会议"]),
        ("employment", "雇佣关系", ["招聘", "面试", "试用期", "劳动合同", "辞职", "薪资", "提成"]),
    ], stage="全书", sort=60),
    _c("business_knowledge", "商业经营", "relation", [
        ("operations", "经营运作", ["客户", "供应商", "渠道", "经销", "库存", "成本", "报价"]),
        ("transaction", "交易谈判", ["合同", "签约", "条款", "报价", "谈判", "竞标", "收购"]),
        ("company_governance", "公司治理", ["股东", "董事会", "股权", "股份", "表决", "分红"]),
    ], stage="全书", sort=70),
    _c("finance_knowledge", "资金财务", "state", [
        ("financing", "融资借贷", ["融资", "贷款", "抵押", "授信", "担保", "利息"]),
        ("cashflow", "现金流", ["现金流", "账期", "回款", "应收", "应付", "周转"]),
        ("investment", "投资风险", ["投资", "估值", "收益", "风险", "止损", "持股"]),
    ], stage="全书", sort=80),
    _c("legal_knowledge", "法律实务", "state", [
        ("contract_evidence", "合同证据", ["合同", "证据", "签字", "盖章", "委托书", "录音"]),
        ("dispute_process", "纠纷程序", ["律师", "起诉", "法院", "立案", "庭审", "调解", "判决"]),
        ("rights_duties", "权利义务", ["权利", "义务", "违约", "赔偿", "授权", "保密"]),
    ], stage="全书", sort=90),
    _c("media_knowledge", "传媒娱乐", "state", [
        ("talent_training", "艺人培养", ["练习生", "训练", "声乐", "舞蹈", "考核", "出道"]),
        ("production", "内容制作", ["录音", "专辑", "拍摄", "剪辑", "制作人", "编曲"]),
        ("promotion", "发行宣传", ["宣传", "发行", "打歌", "通告", "电视台", "榜单", "经纪"]),
    ], stage="全书", sort=100),
    _c("urban_life", "城市生活", "state", [
        ("housing", "住房物业", ["租房", "房租", "押金", "中介", "物业", "小区"]),
        ("transport", "交通出行", ["地铁", "公交", "打车", "停车", "通勤", "航班"]),
        ("public_service", "公共服务", ["医院", "学校", "银行", "派出所", "社保", "办证"]),
    ], stage="全书", sort=110),
]

XIANYAN = [
    _c("family_system", "家庭关系", "relation", [
        ("generation", "代际边界", ["父母", "公婆", "岳父", "岳母", "赡养", "分家"]),
        ("sibling", "手足资源", ["兄弟", "姐妹", "偏心", "家产", "照顾", "借钱"]),
        ("household_role", "家庭分工", ["家务", "照料", "工资", "开支", "带孩子", "做饭"]),
    ], stage="全书", sort=5),
    _c("marriage_life", "婚姻生活", "state", [
        ("registration", "婚姻程序", ["结婚证", "领证", "介绍信", "登记", "离婚", "调解"]),
        ("shared_finance", "共同财务", ["工资", "存折", "彩礼", "嫁妆", "共同开支", "欠款"]),
        ("residence", "居住安排", ["婚房", "宿舍", "合租", "搬家", "户口", "随军"]),
    ], stage="全书", sort=8),
    _c("era_life", "年代生活", "state", [
        ("ration", "票证供应", ["粮票", "布票", "肉票", "供应", "供销社", "定量"]),
        ("work_unit", "单位制度", ["单位", "工分", "知青", "下乡", "招工", "档案"]),
        ("military_family", "军属生活", ["军属", "随军", "家属院", "津贴", "转业", "探亲"]),
    ], stage="全书", sort=9),
    _c("livelihood", "住房生计", "state", [
        ("housing", "住房租住", ["住房", "租金", "押金", "宿舍", "房产", "过户"]),
        ("income", "收入支出", ["工资", "奖金", "存款", "生活费", "学费", "医药费"]),
        ("small_business", "个体经营", ["摆摊", "进货", "卖货", "成本", "生意", "营业执照"]),
    ], stage="全书", sort=11),
    _c("work_social", "工作社交", "state", [
        ("employment", "工作流动", ["招聘", "调岗", "辞职", "升职", "合同", "转正"]),
        ("social_boundary", "人情边界", ["借钱", "担保", "介绍", "人情", "邻居", "同事"]),
    ], stage="全书", sort=12),
    _c("public_service", "公共服务", "state", [
        ("education", "入学教育", ["入学", "学籍", "考试", "学校", "报名", "毕业"]),
        ("medical", "就医照护", ["挂号", "住院", "病历", "检查", "手术", "陪护"]),
        ("civil", "户籍办事", ["户口", "介绍信", "证明", "派出所", "街道", "登记"]),
    ], stage="全书", sort=13),
...
MOSHI = [
    ...
    _c("survival_practice", "求生行动", "state", [
        ("water_food", "饮水食物", ["净水", "煮沸", "饮水", "食物", "配给", "搜集"]),
        ("mobility", "撤离转移", ["撤离", "路线", "车辆", "绕行", "转移", "侦察"]),
        ("tools", "工具使用", ["工具", "绳索", "撬棍", "照明", "地图", "容器"]),
    ], stage="全书", sort=25),
    _c("base", "基地据点", "state", [
        ("shelter", "避难所", ["避难所", "基地", "据点", "安全区", "营地"]),
        ("defense", "防御工事", ["围墙", "路障", "岗哨", "加固"]),
        ("infrastructure", "基础设施", ["发电", "供水", "排水", "通风", "仓库", "通讯"]),
    ], stage="中段", sort=30),
    _c("logistics", "物资后勤", "state", [
        ("storage", "储存保管", ["储存", "仓库", "密封", "保质期", "受潮", "腐坏"]),
        ("rationing", "配给盘点", ["配给", "定量", "盘点", "库存", "消耗", "份额"]),
        ("transport", "运输维护", ["运输", "车队", "油料", "维修", "零件", "补给线"]),
    ], stage="全书", sort=35),
    _c("health", "医疗卫生", "state", [
        ("infection_control", "感染隔离", ["隔离", "感染", "伤口", "消毒", "观察期", "防护"]),
        ("sanitation", "营地卫生", ["厕所", "污水", "垃圾", "清洁", "卫生", "腐烂"]),
        ("treatment", "伤病处置", ["止血", "包扎", "药品", "抗生素", "手术", "高烧"]),
    ], stage="全书", sort=40),
    _c("ecology", "环境生态", "state", [
        ("contamination", "污染风险", ["污染", "辐射", "毒气", "水源", "土壤", "粉尘"]),
        ("weather", "天气威胁", ["暴雨", "严寒", "高温", "风暴", "积雪", "洪水"]),
        ("food_chain", "变异生态", ["变异植物", "变异兽", "食物链", "繁殖", "迁徙", "领地"]),
    ], stage="全书", sort=45),
    _c("team", "队伍", "relation", [
    ...
    _c("order", "秩序变化", "state", [
        ...
        ("labor", "劳动分工", ["分工", "值守", "巡逻", "种植", "维修", "贡献"]),
        ("justice", "惩戒裁决", ["处罚", "审判", "驱逐", "监禁", "仲裁", "违规"]),
    ], stage="中后段", sort=60),
    _c("threat_rule", "威胁规律", "state", [
        ("infected_behavior", "感染体行为", ["丧尸", "感染者", "听觉", "嗅觉", "夜间", "聚集"]),
        ("mutation_limit", "变异限制", ["变异", "进化", "弱点", "惧怕", "无法", "范围"]),
        ("human_risk", "人为风险", ["劫掠", "设卡", "诱骗", "黑市", "垄断", "扣押"]),
    ], stage="全书", sort=65),
...
WUXIANLIU = [
    ...
    _c("info_clue", "规则线索", "object_anomaly", [
        ("document", "文字记录", ["纸条", "日记", "公告", "说明", "档案", "提示"]),
        ("behavior", "行为验证", ["试探", "验证", "观察", "反应", "重复", "异常"]),
        ("environment", "环境提示", ["钟声", "灯光", "门牌", "房间", "路线", "时间"]),
    ], stage="全书", sort=25),
    _c("task", "任务", "event", [
    ...
    _c("resource_exchange", "资源兑换", "state", [
        ("currency", "积分货币", ["积分", "生存点", "货币", "兑换", "价格", "结算"]),
        ("item", "道具权限", ["道具", "次数", "绑定", "携带", "权限", "使用条件"]),
    ], stage="全书", sort=72),
    _c("transition", "世界转场", "state", [
        ("entry", "进入条件", ["进入", "传送", "倒计时", "集合", "门", "载入"]),
        ("return", "返回结算", ["返回", "结算", "休息", "下一场", "冷却", "现实世界"]),
    ], stage="全书", sort=75),
    _c("hook", "章节钩子", "hook", [("chapter_end", "章末悬念", [])], stage="章末", sort=80),
]
...
ZHONGTIAN = [
    ...
    _c("skill", "谋生手艺", "state", [
        ...
        ("gathering", "采集渔猎", ["野菜", "山货", "采集", "捕鱼", "打猎"]),
        ("tools", "农具使用", ["犁", "耙", "锄", "镰", "农具", "修理"]),
    ], stage="全书", sort=20),
    _c("business", "经营发家", "event", [
        ("first_money", "第一桶金", ["第一笔", "卖了", "赚", "换钱", "银子"]),
        ("expand", "扩大经营", ["铺子", "作坊", "田地", "雇", "长工", "分号"]),
        ("market", "集市交易", ["赶集", "摆摊", "牙人", "行市", "过秤", "摊位"]),
        ("prices", "物价收支", ["物价", "工钱", "租金", "成本", "损耗", "账目"]),
        ("side_business", "家庭副业", ["副业", "编织", "纺织", "养蚕", "卖货", "加工"]),
    ], stage="中段", sort=30),
    _c("daily", "日常细节", "state", [
        ("food", "吃食", ["米", "粥", "腊肉", "野菜", "红薯", "包子", "腌"]),
        ("chore", "劳作", ["下地", "喂", "砍柴", "挑水", "洗", "缝"]),
        ("household", "家庭分工", ["当家", "长子", "儿媳", "分家", "养老", "家用"]),
        ("village", "村庄院落", ["村庄", "院落", "正房", "灶房", "水井", "祠堂"]),
        ("custom", "婚育民俗", ["婚嫁", "聘礼", "生育", "满月", "节庆", "祭祖"]),
    ], stage="全书", sort=40),
    _c("weather_season", "天气节气", "state", [
        ("weather", "天气变化", ["下雨", "暴雨", "干旱", "霜", "冰雹", "大雪", "回暖", "降温"]),
        ("season", "时令节气", ["立春", "清明", "谷雨", "芒种", "夏至", "白露", "秋分", "霜降"]),
        ("farm_timing", "农时", ["春耕", "夏收", "秋收", "冬藏", "抢收", "播种时节", "误了农时"]),
        ("disaster", "灾荒应对", ["旱灾", "洪灾", "蝗灾", "逃荒", "赈济", "断粮"]),
    ], stage="全书", sort=41),
    _c("crop_cultivation", "作物种植", "state", [
        ("crop", "作物品种", ["水稻", "小麦", "玉米", "高粱", "豆", "棉花", "油菜", "果树"]),
        ("sowing", "播种育苗", ["播种", "育苗", "催芽", "插秧", "移栽", "间苗", "补苗"]),
        ("field_care", "田间管理", ["浇水", "灌溉", "施肥", "除草", "松土", "追肥", "轮作"]),
        ("pest", "病虫害", ["虫害", "蝗虫", "蚜虫", "病害", "烂根", "枯叶", "农药"]),
        ("harvest", "收获储藏", ["收割", "脱粒", "晾晒", "入仓", "储藏", "发霉", "粮仓"]),
        ("crop_use", "作物用途", ["主粮", "饲料", "榨油", "纤维", "药用", "轮作"]),
    ], stage="全书", sort=42),
    _c("soil_water", "土壤水利", "state", [
        ("soil", "土壤地力", ["黑土", "沙地", "盐碱", "肥力", "地力", "沃土", "贫瘠"]),
        ("water", "水源灌渠", ["水源", "水井", "河渠", "沟渠", "水车", "蓄水", "排涝"]),
        ("land_tenure", "田地制度", ["田界", "地契", "佃租", "租税", "开荒", "典卖"]),
    ], stage="全书", sort=43),
    _c("livestock_processing", "养殖加工", "state", [
        ("livestock", "家禽牲畜", ["鸡", "鸭", "鹅", "猪", "牛", "羊", "牲口", "鸡舍", "猪圈"]),
        ("breeding", "饲养繁育", ["饲料", "喂养", "孵化", "配种", "产崽", "防疫", "兽医"]),
        ("processing", "农产加工", ["磨面", "榨油", "酿酒", "腌制", "晒干", "熏制", "作坊"]),
        ("storage", "储藏保鲜", ["粮仓", "地窖", "晾晒", "防潮", "腌渍", "发霉"]),
    ], stage="全书", sort=44),
