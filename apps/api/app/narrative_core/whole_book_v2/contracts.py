...
class EvidenceRef(M):
    ...
    def range_ok(self):
        ...
        return self
class TypeProfile(M):
    version:str="2.0"; primary_genre:str; secondary_genres:list[str]; narrative_drivers:list[str]; narrative_traits:list[str]; genre_confidence:float=Field(ge=0,le=1); analysis_focus:list[str]; evidence:list[str]
class Ranged(M):
...
class PacingMarker(M): chapter:int=Field(ge=1); title:str; event:str; importance:float=Field(ge=0,le=1); effect_on_pacing:str; evidence:list[str]
...
class AssessmentIssue(Ranged): issue_id:str; priority:Literal["P0","P1","P2"]; category:str; symptom:str; root_cause:str; reader_impact:str; supporting_metrics:list[str]; possible_direction:str
...
class AssessmentResult(M): version:str="2.0"; overall_summary:str; dimensions:list[AssessmentDimension]; strengths:list[Strength]; issues:list[AssessmentIssue]; issue_map:list[AssessmentIssue]; revision_priorities:list[RevisionPriority]; preserve_list:list[str]
...
V2_STAGES=["prepare_source","type_profile","build_windows","extract_primitives","normalize_characters","story_structure","storylines","protagonist_arc","relationships","suspense","causal_chronology","pacing","assessment","evidence_validation","materialize_report"]
