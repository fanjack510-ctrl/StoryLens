...
test.describe("Phase 1C-C.2.5 Reader Journey viewport fit", () => {
  ...
  test("embedded books: single journey analysis, resize, evidence, export", async ({ page }) => {
    ...
    await expect(page.getByTestId("journey-question-chains")).toHaveCount(0);
    ...
    await expect(page.getByTestId("journey-resize-handle")).toBeVisible();
    await page.getByTestId("journey-reset-layout").click();
  ...
  test("independent results route supports overview=curve", async ({ page }) => {
    await mockApis(page);
    await page.goto(
    ...
    await expect(page.getByTestId("journey-sync-workspace")).toBeVisible({ timeout: 15000 });
    await expect(page.getByTestId("journey-overview-curve")).toBeVisible();
    await expect(page.getByTestId("journey-resizable-split")).toBeVisible();
  });
});
