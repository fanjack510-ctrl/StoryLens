...
$Repo = "D:\Dstorylens-wt-hotfix-1.1.2-integration"
$Acceptance = Join-Path $Repo "release\evidence\hotfix\1.1.2\CHG-20260731-023\acceptance"
$MgRoot = Join-Path $env:TEMP "storylens-rc7-chg023-browser"
...
try {
    ...
    Write-Host "`n=== Reseed + Restart API + brief case B (Case G) ==="
    Stop-ProcSafe $apiProc
    Stop-PortListener -Port $ApiPort
    Start-Sleep -Seconds 2
    Invoke-Seed
    ...
    Write-Host "`n=== CHG-023 RC.7 browser E2E PASSED ==="
